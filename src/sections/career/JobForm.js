...
import {
  Anchor, Box, Button, CheckboxGroup, Flex, Grid, GridCol, Text, Title,
} from '@mantine/core'
...
import {
  validNameInput, validEmail, validPhoneNumber, validURL,
  validURLNotHTTP,
} from '@/common/regex'
...
function validateField(value, required, validateType, validateError, maxLength = null, maxLengthError = '', minLength = null, minLengthError = '') {
...
function CareerSubmissionFormPage(props) {
  ...
  const disabledSubmitButton = () => disabledSubmit || (!privacy || !consent2) || disableAllFields
  ...
  const form = useForm({
    ...
    validate: {
      firstname: (value) => validateField(value, true, validNameInput, 'Do not support special characters', 50, 'Limit maximum input field'),
      lastname: (value) => validateField(value, true, validNameInput, 'Do not support special characters', 50, 'Limit maximum input field'),
      email: (value) => validateField(value, true, validEmail, 'Email must be formatted correctly.', 50, 'Limit maximum input field'),
      phoneNumber: (value) => validateField(value, true, validPhoneNumber, 'Can input only number', 50, 'Limit maximum input field', 9, 'The number must be at least 9 digits.'),
      cvLink: (value) => {
      ...
      acceptConsent1: (value) => (_isEmpty(value) ? 'Please complete required field.' : null),
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const base64FilesPromises = _map(fileUploaded, (file) => convertFileToBase64(file))
    ...
    >
      ...
      >
        ...
        >
          ...
          <Box mb="1.5rem">
            <Title c="lightBlue.2" fz="22px">{title}</Title>
            <Text c="darkBlue.0">
              <Image
          ...
          <Grid mb="1.5rem" gutter="xl">
            ...
            <GridCol span={{ base: 12, sm: 6 }} display="flex" style={{ alignItems: 'flex-end', gap: '32px' }}>
              <Text>or</Text>
              <UploadFile
                ...
                onChange={(f) => {
                  if (f?.type !== 'application/pdf') {
                    ...
                    setCvFile(null)
                    form.setFieldError('cvLink', 'File size exceeds maximum limit (5 Mb).')
                  } else {
                    setCvFile(f)
                  }
                }}
              ...
              <UploadFile
                ...
                onChange={(f) => {
                  if (f?.type !== 'application/pdf') {
                    ...
                    setPortfolioFile(null)
                    form.setFieldError('portfolioLink', 'File size exceeds maximum limit (5 Mb).')
                  } else {
                    setPortfolioFile(f)
                  }
                }}
          ...
          <Box m="60px 0 40px">
            <Text mb="1rem">
              1. Collection, use and disclosure of personal data in order to inform news,
              promotion and presentation of products and services of the Company,
              its business partners and other juristic entities.*
            </Text>
            <CheckboxGroup
              mb="10px"
              styles={{
            ...
            >
              <Flex gap={{ base: '20px', md: '40px' }} direction={{ base: 'column', sm: 'row' }}>
                <Checkbox
                  label="Consent"
                  value
                  disabled={disableAllFields}
          ...
          <Box>
            <Text c="lightBlue.0">
              As the Company is required to collect,
              use and disclose my sensitive personal data for human resource management purposes,
              if I do not consent to this clause, the Company may not be able to perform its duties
              or obligations under the employment contract or contract of hiring services,
              such as the Company may not be able to proceed with my request,
              I may face any inconvenience and/or I may receive damage or lose the opportunity to
              receive the relevant benefits.
            </Text>
          </Box>
          <Box my="20px">
            <Flex gap="15px">
              ...
              />
              <span style={{ width: '80%' }}>
                I have read and understood the above details of requesting for consent information
                as set out in the Company’s Privacy Policy. For details, please read the
                {' '}
                <Anchor td="underline" href="/pages/privacy-policy-hr.html">privacy policy</Anchor>
                *
              </span>
            </Flex>
            {privacyError && <Text mb="20px" c="#9c0d0d">Please complete required field.</Text>}
          </Box>
